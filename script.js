/* =========================================
   CHRONOS VELOCITY ENGINE v0.1.7 (PATCHED)
   Fixed: ReferenceErrors via Re-Ordering
   ========================================= */

// --- 0. GLOBAL SAFETY NET ---
window.onerror = function(message, source, lineno, colno, error) {
    const out = document.getElementById('game-output') || document.body;
    out.innerHTML += `<div style="color:red; border:1px solid red; padding:10px; margin:10px; background:black; z-index:9999;">
        <strong>SYSTEM CRASH:</strong> ${message}<br>
...
const AccessibilityManager = {
    init() { console.log("[SYS] A11Y: READY"); },
    speak(text) { /* Screen Reader Hook */ },
...
const AudioController = {
    ...
    init() {
        ...
        this.reverbNode.buffer = this.createImpulse(2.0, 2.0);
    ...
    toggleMute() {
        ...
        this.masterGain.gain.linearRampToValueAtTime(this.isMuted ? 0 : 0.4, now + 0.1);
        return this.isMuted;
    },
    ...
    playType() { this.playTone(800, 'triangle'); },
    playAI() { this.playTone(110, 'sawtooth'); },
    playBlip() { this.playTone(440, 'sine'); }
};
...
const InputProcessor = {
    processBio(type, rawVal) {
        ...
        if (type === 'height') {
            ...
            if (match && val.includes("'")) { cm = (parseInt(match[1]) * 30.48) + (parseInt(match[2] || 0) * 2.54); }
            ...
            else if (val.includes('usa')) response = "USA? Metric conversion active. 🦅";
            else response = `${rawVal}? Exotic. Downloading database... 🌍`;
        }
...
// Inject audio button if missing
...
let gameState = {
    ...
    inventory: [], history: [], rules: null, story: null, storyActive: false,
    waitingForEnter: false, waitingForChoice: false, currentInputType: null, currentChoices: null,
    driver: '', passenger: '', citationsShown: false, isTyping: false, skipTyping: false,
    typingSpeed: 25, typingSpeedAI: 40
};

// --- 3. CORE GAME FUNCTIONS (Moved UP to fix ReferenceErrors) ---
...
function applyClassSelection(val, target) {
    ...
    if (!gameState.rules) {
        printLine("ERROR: Rules not loaded.", 'system');
        return false;
    }
...
function startGame() {
    ...
    // Safety check
    if (!gameState.story) {
        printLine("BUFFERING DATA...", 'system');
...
async function init() {
    ...
    // Audio Hooks
    const startAudio = () => AudioController.init();
    ...
    if(audioBtn) audioBtn.addEventListener('click', () => {
        ...
        const icon = document.getElementById('audio-icon') || audioBtn;
...
function handleStoryInput(val) {
    ...
    if (response) {
        printLine(response, 'ai').then(() => { setTimeout(playNextScene, 1000); });
    } else {
        playNextScene();
    }
}
...
function typeWriter(element, html, speed) {
    return new Promise((resolve) => {
        AccessibilityManager.speak(html);
        ...
        function finishTyping() { element.classList.remove('typing'); element.classList.add('typed-done'); gameState.isTyping = false; output.scrollTop = output.scrollHeight; setInputReady(true); }
        typeNode();
    });
}
...
function resolveTextVariant(blocks, focus) {
    let bestBlock = null; let highestPriority = -1; let target = (focus === 'partner') ? gameState.partner : gameState.player;
    const checkCondition = (cond) => {
        ...
        if (cond.startsWith('synergy:')) { const [s1, s2] = cond.split(':')[1].split('_'); const map = { tech:'Tech', arts:'Arts', guts:'Guts', social:'Social', bio:'Bio', lore:'Lore' }; if (target.stats[map[s1]] >= 5 && target.stats[map[s2]] >= 5) return 9; }
        return -1;
    };
...
function setInputReady(isReady) { const dot = document.getElementById('input-status'); if(dot) { if(isReady) dot.classList.add('status-ready'); else dot.classList.remove('status-ready'); } }
